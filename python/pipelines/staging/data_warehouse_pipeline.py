...
class DataWarehousePipeline(Pipeline):
    ...
    def delete_temp_tables(self):
        ...
        self.snowflake_handler.run_raw_query('DELETE FROM "Temp_DateTime";')
        self.snowflake_handler.run_raw_query('DELETE FROM "Temp_Cause";')
        self.snowflake_handler.run_raw_query('DELETE FROM "Temp_Stations";')
        self.snowflake_handler.run_raw_query('DELETE FROM "Temp_Disruptions";')
        self.snowflake_handler.run_raw_query('DELETE FROM "Temp_Station_Lines";')
    ...
    def _load(self):
        ...
        """
        config = {"if_exist": "append", "chunksize": 16000}
    ...
    def _extract(self):
        ...
        self.fact_disruption = self.sql_extracter.extract('SELECT * FROM "Fact_Disruptions";')
    ...
    def _transform(self):
        try:
            ...
            # Transform dates
            self.transformers.merge_data(self.dim_date, "Temp_DateTime", "Dim_DateTime", ["id"], ["id"])
            ...
            # Transform Fact
            self.transformers.merge_data(self.fact_disruption, "Temp_Disruptions", "Fact_Disruptions", ["rdt_id"], ["rdt_id"])
            print("Transformed Fact")
            # Transform line_station
            self.transformers.merge_data(self.dim_line_station, "Temp_Station_Lines", "Dim_Station_Lines", ["rdt_id", "rdt_lines_id"], ["rdt_id", "rdt_lines_id"])

...
from python import MissingEndDateTransformer, ColumnSplitter, DateKeyHandler, StringRemover
...
def test_column_splitter_incorrect_vertical_split_last_value_missing():
    splitter = ColumnSplitter()
    record_one = {"rdt_id": "40500", "rdt_lines": "Den Haag HS - Rotterdam Centraal", "rdt_lines_id": "11"}
    record_two = {"rdt_id": "40504", "rdt_lines": "Alkmaar - Den Helder; Alkmaar - Hoorn", "rdt_lines_id": "162"}
    dff = pd.DataFrame([record_one, record_two]) 

    with pytest.raises(ValueError, match="Provided columns that need to split vertically must need to equal to eachother"):
        splitter.split_column_vertically(dff, ["rdt_lines", "rdt_lines_id"], ",")
...
def test_string_remover():
    ...
    record_one = {"rdt_id": "40500", "rdt_lines": "Den Haag HS (HSL) - Rotterdam Centraal (HSL)", "rdt_lines_id": "11"}
    record_two = {"rdt_id": "40500", "rdt_lines": "Den Haag HS (HSL) - Rotterdam Centraal", "rdt_lines_id": "11"}
    string_remover.remove()
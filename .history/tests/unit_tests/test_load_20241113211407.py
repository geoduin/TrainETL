...
def reset_database():
    ...
    handler.run_raw_query('DROP TABLE IF EXISTS "Person"')
...
"""
    with handler.get_connection() as con:
        con.execute(schema)
        answer = df.to_sql("Person", con=connection, if_exists="append", index=False)
        assert answer == 1

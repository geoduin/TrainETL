...
class SCDType1Transformer(SCDTransformer):
    ...
    def merge_data(self, data, source, target):
        try:
            ...
            upper_columns = [col.upper() for col in data.columns]
            set_values = ", ".join([f"t.{col} = s.{col}" for col in upper_columns])
            column_values = ", ".join(upper_columns)
            insert_values = ", ".join([f"s.{col}" for col in upper_columns])
            query = f"""

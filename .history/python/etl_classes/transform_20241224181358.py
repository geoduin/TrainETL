...
from ..service.SQLHandler import SnowflakeHandler
...
class ConvertionTransformer(Transform):
    ...
    def run(self, data):
        ...
        # Drop duplicates
        line_stations = line_stations.drop_duplicates(subset=['begin_station', 'end_station'])
        cause_table = cause_table.drop_duplicates(subset=['cause_id'])
        station_table = station_table.drop_duplicates(subset=['id'])
...
class SCDTransformer(Transform):
    ...
    """
    def __init__(self, sql_handler: SnowflakeHandler):
        self.next = None
        self.snowflake_handler = sql_handler
...
class SCDType1Transformer(SCDTransformer):
    ...
    It will ensure the migration of new data to the target table.
    ...
    def merge_data(self, data, source, target, source_key="id", target_key="id"):
        try:
            # Validate if columns source and target are the same
            uppercase = lambda x: x.upper() (data.columns)
            set_values = ", ".join([f"t.{col} = s.{col}" for col in data.columns])
            column_values = ", ".join(data.columns)
            insert_values = ", ".join([f"s.{col}" for col in data.columns])
            query = f"""
                ...
                """
            self.snowflake_handler.run_raw_query(query)
...
class FactInserter(Transform):
    ...
    def run(self, data: DataFrame, fact_table=None) -> DataFrame:

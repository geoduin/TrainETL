...
class MissingEndDateTransformer(Transform):
    ...
    def run(self, data: DataFrame) -> DataFrame:
        ...
        data["duration"] = data.apply(lambda row: self._calculate_duration(row['startdate'], row["enddate"]), axis=1)
        return super().run()
    ...
    def _fill_current_date(self, data: DataFrame) -> DataFrame:
        ...
        """
        data["enddate"].fillna(datetime.now())
        return data["enddate"].fillna(datetime.now())
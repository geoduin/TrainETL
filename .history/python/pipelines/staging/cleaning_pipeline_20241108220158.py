...
class CleaningPipeline(Pipeline):
    ...
    def run(self):
        ...
        try:
            ...
            services = self.sql_extracter.extract('SELECT * FROM "Disruptions"')
            ...
            self.sql_load.load_data(disruptions, "Stations", **config)
            self.sql_load.load_data(disruptions, "Distances", **config)

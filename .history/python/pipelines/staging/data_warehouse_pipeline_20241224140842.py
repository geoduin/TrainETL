...
from sqlalchemy import Table, Column, Integer, String, MetaData, create_engine, ForeignKey, Float
...
class DataWarehousePipeline(Pipeline):
    ...
    def _create_tables_ifnotexists(self):
        ...
        dim_lines = Table("Dim_Station_Lines", 
            METADATA, 
            Column("rdt_id", Integer, primary_key=True, foreign_key=ForeignKey("Fact_Disruptions.id")), 
            Column("rdt_lines_id", String),
            Column("rdt_lines", String),
            Column("begin_station", Integer, foreign_key=ForeignKey("Dim_Stations.id")),
            Column("end_station", Integer, foreign_key=ForeignKey("Dim_Stations.id")))   
        fact_disruptions = Table("Fact_Disruptions", 
            METADATA, 
            Column("id", Integer, primary_key=True), 
            Column("start_time", Integer, foreign_key=ForeignKey("Dim_DateTime.id")),
            Column("end_time", Integer, foreign_key=ForeignKey("Dim_DateTime.id")),
            Column("duration_minutes", Integer),
            Column("cause_id", Integer, foreign_key=ForeignKey("Dim_Cause.cause_id")))

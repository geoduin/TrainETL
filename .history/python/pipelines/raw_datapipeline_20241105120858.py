...
class RawPipeline(Pipeline):
    ...
    def run(self):
        ...
        try:
            ...
            self.sql_load.load_data(stations, "Stations", config=config)
            self.sql_load.load_data(disruptions, "Disruptions", config=config)
            self.sql_load.load_data(distances, "Distances", config=config)

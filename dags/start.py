...
from raw_pipeline.delivery_dag import DeliveryDag

from python import RawPipeline, CSVExtract, SQLLoad, CleaningPipeline, SQLExtracter, MissingEndDateTransformer, SQLHandler, ConvertionPipeline, ConvertionTransformer, DateKeyHandler, ColumnSplitter, StringRemover, DataWarehousePipeline, SnowflakeLoad, SCDType1Transformer, SnowflakeHandler
...
datawarehouse_connection = os.environ.get("SNOWFLAKE_CONNECTION", None) 

# Extract classes
csv_extracter = CSVExtract()
sql_extracter = SQLExtracter(raw_connection)
sql_extracter_staging = SQLExtracter(staging_connection)
sql_extracter_convertion = SQLExtracter(convertion_connection)
...
snowflake_manual_handler = SnowflakeHandler(datawarehouse_connection)
snowflake_scd_transformer = SCDType1Transformer(snowflake_manual_handler)
...
data_warehouse_pipeline = DataWarehousePipeline(sql_extracter_convertion, snowflake_loader, snowflake_scd_transformer, snowflake_manual_handler)
...
delivery_dag = DeliveryDag("deliver_data", start_date=datetime(2024, 11, 18), schedule_interval="@daily")
